---
title: "Effect of CMV on CD4N_pct clones"
...
columns_to_check =c('CMV_status', 'age', 'sex', 'race', 'CD4N_pct')
filtered_df <- data_df %>% 
...
### **Investigating CMV's impact on CD4 naive T cells pct**
#### **Finding the unadjusted mean difference**

```{r}
## finding un-adjusted ATE
CD4N_pct_t = filtered_df$CD4N_pct[filtered_df$CMV_status == 1]
CD4N_pct_c = filtered_df$CD4N_pct[filtered_df$CMV_status == 0]

ATE = mean(CD4N_pct_t, na.rm = T) - mean(CD4N_pct_c, na.rm = T)

# finding the weighted standard error
SE = sqrt(var(CD4N_pct_t, na.rm = T)/length(CD4N_pct_t) + var(CD4N_pct_c, na.rm = T)/length(CD4N_pct_c))

# finding the confidence interval
CI = ATE + c(-1, 1) * qnorm(0.975) * SE

# test for significance
p_val = t.test(CD4N_pct_t, CD4N_pct_c, var.equal = TRUE)$p.value
...
kable(output1, caption = "Unadjusted ATE of CMV on CD4N_pct", align = "c")
```

We can conclude that there is a statistically significant differences in the CD4N_pct clone count between CMV positive and negative individuals.
...
Average Treatment Effect of CMV on CD4N_pct clones
```{r}
...
model_formula = CD4N_pct ~ CMV_status * age + sex + race + educ + smoke_stat + INFLAM_5 + comorb_idx + Tcells
...
kable(result_1, caption = "CD4N_pct: the average treatment effect (ATE)", align = "c")
```

Average Treatment Effect of the Treated due to CMV on CD4N_pct clones
```{r}
...
kable(result_1_1, caption = "CD4N_pct: ATT", align = "c")
```
...
mod_ps = glm(CMV_status ~ age + sex + race + educ + smoke_stat + INFLAM_5 + comorb_idx, family = binomial(), data = filtered_df2)
...
# num_cols = c('age', 'educ', 'BMI', 'INFLAM_5', 'comorb_idx', 'Tcells', 'CD4N_pct', 'CD4N_pct')
...
te_quintile = tapply(filtered_df2$CD4N_pct[filtered_df2$CMV_status == 1],
                      ps_quintile[filtered_df2$CMV_status == 1], mean) -  
               tapply(filtered_df2$CD4N_pct[filtered_df2$CMV_status == 0],
...
for(i in 1:100) {
  ...
  # finding TE for each quintile
  te_quintile_boot = tapply(filtered_boot$CD4N_pct[filtered_boot$CMV_status == 1],
                             ps_quintile_boot[filtered_boot$CMV_status == 1], mean) -
                     tapply(filtered_boot$CD4N_pct[filtered_boot$CMV_status == 0],
...
for(i in 1:100) {
  ...
  # finding TE of the treated for each quintile
  te_quintile = tapply(filtered_boot$CD4N_pct[filtered_boot$CMV_status == 1],
                        ps_quintile_boot[filtered_boot$CMV_status == 1], mean) -  
                 tapply(filtered_boot$CD4N_pct[filtered_boot$CMV_status == 0],
                        ps_quintile_boot[filtered_boot$CMV_status == 0], mean)
...
# compute IPW ATE
Y1 = filtered_df2$CD4N_pct[filtered_df2$CMV_status == 1]
Y0 = filtered_df2$CD4N_pct[filtered_df2$CMV_status == 0]
...
for (i in 1:n_boot) {
  ...
  Y1b = boot_df$CD4N_pct[boot_df$CMV_status == 1]
  Y0b = boot_df$CD4N_pct[boot_df$CMV_status == 0]
...
# finding TE for each quintile
te_quintile = tapply(matched_df$CD4N_pct[matched_df$CMV_status == 1],
                      ps_quintile[matched_df$CMV_status == 1], mean) -  
               tapply(matched_df$CD4N_pct[matched_df$CMV_status == 0],
...
for(i in 1:100) {
  ...
  # finding TE for each quintile
  te_quintile_boot = tapply(matched_boot$CD4N_pct[matched_boot$CMV_status == 1],
                             ps_quintile_boot[matched_boot$CMV_status == 1], mean) -
                     tapply(matched_boot$CD4N_pct[matched_boot$CMV_status == 0],
...
for(i in 1:100) {
  ...
  # finding TE of the treated for each quintile
  te_quintile = tapply(matched_boot$CD4N_pct[matched_boot$CMV_status == 1],
                        ps_quintile_boot[matched_boot$CMV_status == 1], mean) -  
                 tapply(matched_boot$CD4N_pct[matched_boot$CMV_status == 0],
                        ps_quintile_boot[matched_boot$CMV_status == 0], mean)
